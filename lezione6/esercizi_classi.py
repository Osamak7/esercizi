 ...
 #   def __init__(self, name:str , grades:list[float] =[]) -> None:
  #      self.name=name
   ...
   # def get_avg_grade(cls):
        #if len(cls.all_grades) != 0:
          #  return sum(cls,all_grades)/ len (cls.all_grades)
#student=Student(name="Lorenzo",grades=[1,2,3])
#student1=Student(name="Luca", grades=[4,5,6])
...
class Resturant :
    ...
    def Describe_resturant(self):
        print (f"{self.resturant_name} e un ristorante di cucina {self.cuisine_type}")
    
    def Open_resturant(self):
        print(f"il ristorante {self.resturant_name} è aperto")
        
rist1=Resturant("Ali kebbab" , "Kebbabberia")

rist1.Describe_resturant()
rist1.Open_resturant()
